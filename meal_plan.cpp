...
/* this function returns the cost of the meal package in meal_packages that has the smallest
...
std::pair<int, int>* meal_packages = {{20, 142}, {40, 276}, {60, 392}, {80, 493}}) {
    ...
    while (low < mid) {
        int mid = (low + high) >> 1;
        if (meal_packages[mid + 1].first < meal_packages) low = mid + 1;
        else high = mid;
    }
    return (low < meal_packages.size())? low : -1;
}

/* returns an array of pair<int, int> values where the jth entry is 
